...
@Component({
  ...
  imports: [FormField],
  template: `
    <section class="rounded-xl bg-white p-8 shadow max-w-lg">
      <h1 class="text-2xl font-semibold text-slate-900">Caja</h1>

      @if (cashbox.openShifts.isLoading()) {
        <p class="mt-4 text-slate-500">Cargando turno...</p>
      } @else if (!cashbox.currentShift()) {
        <!-- Sin turno abierto: formulario de apertura -->
        <form class="mt-6 space-y-4" (submit)="onOpen($event)">
          <label class="block">
            <span class="text-slate-700">Monto inicial de efectivo</span>
            <input
              type="number"
              step="0.01"
        ...
        <div class="mt-4 rounded bg-slate-50 p-4">
          ...
          <p class="text-sm text-slate-500">
            Desde {{ cashbox.currentShift()!.opened_at }}
          </p>
        </div>

        <h2 class="mt-6 text-lg font-medium text-slate-900">Movimientos</h2>
          ...
          <ul class="mt-2 divide-y divide-slate-200">
            @for (movement of cashbox.movements.value(); track movement.cash_movement_id) {
              <li class="flex justify-between py-2">
                <span class="text-slate-700">{{ movement.movement_type }}</span>
                <span class="font-medium">{{ formatCurrency(movement.amount) }}</span>
              </li>
            } @empty {
              <p class="py-2 text-slate-500">Sin movimientos todavia.</p>
            }
          </ul>
        }

        <h2 class="mt-6 text-lg font-medium text-slate-900">Cerrar turno</h2>
        <form class="mt-4 space-y-4" (submit)="onClose($event)">
          <label class="block">
            <span class="text-slate-700">Efectivo contado</span>
            <input
              type="number"
              step="0.01"
              class="mt-1 w-full rounded border border-slate-300 px-3 py-2"
